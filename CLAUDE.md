...
- **배포할 때 [sw.js](sw.js) 의 `SW_VERSION` 을 1 올리세요.** 서비스워커는 자기 파일(`sw.js`)의 바이트가 바뀔 때만 '새 버전'으로 인식합니다. 다른 파일만 고치면 브라우저는 갱신을 모르고, 열려 있는 앱(PWA)은 옛 JS 를 계속 들고 있습니다 — 서버 규칙만 새 것이면 "화면엔 버튼이 보이는데 서버가 막는" 상태가 됩니다. 올리면 앱이 스스로 새로고침합니다.
